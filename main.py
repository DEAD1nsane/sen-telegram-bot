...
def parse_text_to_blocks(text: str) -> list:
    ...
    for line in lines:
        ...
        if line.strip().startswith("$$") and line.strip().endswith("$$"):
            flush_buffers()
            blocks.append(InputRichBlockMathematicalExpression(expression=line.strip().strip("$")))
            continue
        ...
        if heading_match:
            ...
            blocks.append(InputRichBlockHeading(text=heading_match.group(2).strip(), level=level))
            continue
        ...
        if line.strip().startswith("|") and line.strip().endswith("|"):
            ...
            table_cells = [InputRichTableCell(blocks=[InputRichBlockParagraph(text=c)]) for c in cells]
        ...
        if list_match:
            ...
            list_buffer.append(InputRichBlockListItem(blocks=[InputRichBlockParagraph(text=item_content)]))
            continue
...
async def free_web_search(query: str) -> str:
    ...
    try:
        async with httpx.AsyncClient() as client:
            res = await client.post("[https://html.duckduckgo.com/html/](https://html.duckduckgo.com/html/)", data={"q": query}, headers=headers, timeout=8.0)
...
async def collapse_message(chat_id: int, message_id: int, delay: int):
    ...
    blocks = [
        InputRichBlockCollapsibleDetails(
            ...
            blocks=[InputRichBlockParagraph(text="Your memory list is hidden to conserve space.")]
        )
    ]
    try:
...
async def handle_help(message: Message):
    blocks = [
        ...
        InputRichBlockCollapsibleDetails(
            ...
            blocks=[
                InputRichBlockList(
                    items=[
                        InputRichBlockListItem(blocks=[InputRichBlockParagraph(text="remember [item],, [item2] - Adds items to memory")]),
                        InputRichBlockListItem(blocks=[InputRichBlockParagraph(text="what do you remember - Displays rules in a formatted list")]),
                        InputRichBlockListItem(blocks=[InputRichBlockParagraph(text="edit [number] [new fact] - Edits a specific rule")]),
                        InputRichBlockListItem(blocks=[InputRichBlockParagraph(text="forget [number],, [number2] - Removes memories")]),
                        InputRichBlockListItem(blocks=[InputRichBlockParagraph(text="forget all - Clears all memory")])
                    ]
                )
            ]
        )
    ...
    if message.chat.type == "private":
        ...
        await message.answer_rich_message(rich_message=InputRichMessage(blocks=blocks), reply_to_message_id=message.message_id)
...
async def handle_what_remember(message: Message):
    ...
    if message.chat.type == "private":
        sent_msg = await message.answer_rich_message(rich_message=InputRichMessage(blocks=blocks))
    else:
        sent_msg = await message.answer_rich_message(rich_message=InputRichMessage(blocks=blocks), reply_to_message_id=message.message_id)
...
async def handle_conversation(message: Message):
    ...
    if is_tagged or is_reply_to_bot or is_private or message.content_type in ["voice", "audio"]:
        ...
        if await redis_client.exists(cooldown_key):
            warn_blocks = [InputRichBlockParagraph(text="Slow down, request limit reached.")]
            if is_private:
                await message.answer_rich_message(rich_message=InputRichMessage(blocks=warn_blocks))
            else:
                await message.answer_rich_message(rich_message=InputRichMessage(blocks=warn_blocks), reply_to_message_id=msg_id)
            return
        ...
        if clean_prompt or replied_context or audio_bytes:
            try:
                ...
                if is_private:
                    await message.answer_rich_message(rich_message=InputRichMessage(blocks=rich_blocks))
                else:
                    await message.answer_rich_message(rich_message=InputRichMessage(blocks=rich_blocks), reply_to_message_id=msg_id)
                ...
                error_blocks = [InputRichBlockParagraph(text="I am currently broken right now, the owner needs to fix me.")]
                if "429" in str(ai_err):
                    error_blocks = [InputRichBlockParagraph(text="Whoa, I'm getting a little overwhelmed! Let me catch my breath for a minute.")]
                
                if is_private:
                    await message.answer_rich_message(rich_message=InputRichMessage(blocks=error_blocks))
                else:
                    await message.answer_rich_message(rich_message=InputRichMessage(blocks=error_blocks), reply_to_message_id=msg_id)
