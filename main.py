...
async def set_interaction(user_id: int, action: str) -> None:
    await redis_client.set(interaction_key(user_id), action, ex=INTERACTION_TTL)
...
async def get_formatted_memories(user_id_str: str) -> str:
    ...
    if not memories:
        return "<h2>Your Memories</h2><p>Your memory list is currently empty.</p>"

    lines = ["<h2>Your Memories</h2>", "<ol>"]
    for memory in memories:
        lines.append(f"<li>{html.escape(memory)}</li>")
    lines.append("</ol>")
    return "".join(lines)
...
# These are Telegram Rich Message buttons, not ordinary InlineKeyboardMarkup buttons.
...
async def edit_menu(callback: CallbackQuery, rich_html: str) -> None:
    ...
    if ephemeral_id is not None:
        ...
        await schedule_menu_delete(chat_id, user_id, ephemeral_id, None)
    else:
        ...
        await schedule_menu_delete(chat_id, user_id, None, message.message_id)
...
async def show_help(message: Message) -> None:
    ...
    # Bot API 10.3 ephemeral commands arrive with ephemeral_message_id.
    if message.ephemeral_message_id is not None:
        await send_menu(
            chat_id,
            user_id,
...
async def handle_memory_view(callback: CallbackQuery):
    ...
    memories_html = await get_formatted_memories(str(callback.from_user.id))
    try:
        await edit_menu(callback, rich_memory_menu(memories_html))
    except Exception as e:
...
async def handle_memory_add(callback: CallbackQuery):
    await set_interaction(callback.from_user.id, "add")
    ...
    try:
        await edit_menu(
            ...
            rich_back_close(
                ...
                "<p>You can send multiple items separated by <code>,,</code>.</p>"
            ),
        )
    except Exception as e:
...
async def handle_memory_edit(callback: CallbackQuery):
    ...
    await set_interaction(callback.from_user.id, "edit_number")
    await callback.answer()

    if not memories:
        body = "<h2>Edit memory</h2><p>You don't have any saved memories yet.</p>"
    else:
        rows = ["<h2>Edit memory</h2><p>Send the number and replacement text.</p><ol>"]
        for memory in memories:
            rows.append(f"<li>{html.escape(memory)}</li>")
        rows.append("</ol><p>Example: <code>2 My new instruction</code></p>")
        body = "".join(rows)
...
async def handle_memory_forget(callback: CallbackQuery):
    memories = await get_memories(str(callback.from_user.id))
    await set_interaction(callback.from_user.id, "forget")
    await callback.answer()

    if not memories:
        body = "<h2>Forget memories</h2><p>Your memory list is already empty.</p>"
    else:
        rows = ["<h2>Forget memories</h2><p>Send one number or several separated by <code>,,</code>.</p><ol>"]
        for memory in memories:
            rows.append(f"<li>{html.escape(memory)}</li>")
        rows.append("</ol><p>Example: <code>1,, 3,, 5</code></p>")
        body = "".join(rows)
...
async def handle_memory_forget_all(callback: CallbackQuery):
    await clear_interaction(callback.from_user.id)
    await callback.answer()
    try:
...
async def handle_confirm_forget_all(callback: CallbackQuery):
    ...
    await redis_client.delete(
        ...
        interaction_key(callback.from_user.id),
    )
...
async def free_web_search(query: str) -> str:
    ...
    try:
        ...
        return "\n\n".join(
            ...
            for x in results
            if x.get("title") or x.get("content")
        )
    except Exception as e:
...
async def handle_conversation(message: Message):
    ...
    action = await get_interaction(message.from_user.id)
    ...
    try:
        ...
        if search_context:
            instructions += (
                "\nWhen using Web Search Context, state the information directly. "
                "If the user asks for links or sources, use HTML links. "
                "If they do not ask for sources, do not include URLs."
            )
        if chat_history:
...
async def configure_commands() -> None:
    commands = [
        BotCommand(
            command="help",
            description="Open the private memory menu",
            is_ephemeral=True,
        ),
    ]
...
async def main():
    ...
    try:
        ...
        print("Ephemeral /help command configured.")
    except Exception as e:
