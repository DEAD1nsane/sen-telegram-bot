...
async def transmit_rich_payload(chat_id: int, blocks: list, reply_to_id: int = None) -> dict:
    """Bypasses standard aiogram send methods to manually interface with Telegram's rich text API endpoints."""
...
def parse_text_to_blocks(text: str) -> list:
    """Statefully tokenizes raw text into raw block dictionaries for the native JSON payload."""
    blocks = []
    lines = text.split('\n')
    ...
    for line in lines:
        if line.strip().startswith("```"):
            if in_code_block:
                ...
                flush_buffers()
                clean_code = "\n".join(code_buffer)
                blocks.append({"type": "paragraph", "text": f"Code Snippet:\n{clean_code}"})
                code_buffer.clear()
            else:
                flush_buffers()
                in_code_block = True
        ...
        if line.strip().startswith("$$") and line.strip().endswith("$$"):
            flush_buffers()
            blocks.append({"type": "mathematical_expression", "expression": line.strip().strip("$")})
            continue
        ...
        if line.strip().startswith("|") and line.strip().endswith("|"):
            ...
            if blocks and blocks[-1].get("type") == "table":
                ...
                blocks.append({"type": "table", "is_bordered": True, "is_striped": True, "rows": [new_row]})
            continue
...
async def send_formatted_memories_as_blocks(message: Message, user_id_str: str):
    """ Directly builds native rich block dicts via code iteration, completely bypassing text parsing. """
    try:
        ...
        if not raw_items:
            blocks.append({"type": "paragraph", "text": "Your memory list is currently empty."})
        else:
            list_items = []
            for item in raw_items:
                mem_text = item.decode('utf-8') if isinstance(item, bytes) else item
                list_items.append({"blocks": [{"type": "paragraph", "text": mem_text}]})
        ...
        if sent_msg_id:
            asyncio.create_task(auto_delete_message(message.chat.id, sent_msg_id, message.message_id, 60))
            
    except Exception as e:
        print(f"Error rendering structured memory blocks: {e}")
        await bot.send_message(chat_id=message.chat.id, text="⚠️ Error loading active memory layout.")
...
async def handle_help(message: Message):
    blocks = [
        ...
        {"type": "list", "items": [
            {"blocks": [{"type": "paragraph", "text": "remember [item],, [item2] - Adds items to memory"}]},
            {"blocks": [{"type": "paragraph", "text": "what do you remember - Displays rules in a formatted list"}]},
            {"blocks": [{"type": "paragraph", "text": "edit [number] [new fact] - Edits a specific rule"}]},
            {"blocks": [{"type": "paragraph", "text": "forget [number],, [number2] - Removes memories"}]},
            {"blocks": [{"type": "paragraph", "text": "forget all - Clears all memory"}]}
        ]}
    ]
...
async def handle_edit(message: Message):
    ...
    sent_msg_id = res.get("result", {}).get("message_id")
    if sent_msg_id:
        asyncio.create_task(auto_delete_message(message.chat.id, sent_msg_id, message.message_id, 10))
...
async def handle_conversation(message: Message):
    ...
    if is_tagged or is_reply_to_bot or is_private or message.content_type in ["voice", "audio"]:
        ...
        if await redis_client.exists(cooldown_key):
            warn_blocks = [{"type": "paragraph", "text": "Slow down, request limit reached."}]
        ...
        if clean_prompt or replied_context or audio_bytes:
            try:
                ...
                error_blocks = [{"type": "paragraph", "text": "I am currently broken right now, the owner needs to fix me."}]
                if "429" in str(ai_err):
                    error_blocks = [{"type": "paragraph", "text": "Whoa, I'm getting a little overwhelmed! Let me catch my breath for a minute."}]
